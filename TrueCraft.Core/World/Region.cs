...
public class Region : IDisposable, IRegion
{
    ...
    public IChunk GetChunk(Coordinates2D position, bool generate = true)
    {
        ...
        if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) start, cached={Cached}", Position, position, Chunks.ContainsKey(position));
        if (!Chunks.ContainsKey(position))
        {
            if (regionFile is not null)
            {
                if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) looking up table", Position, position);
                var chunkData = GetChunkFromTable(position);
                if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) table result null={Null}", Position, position, chunkData is null);
                if (chunkData is null)
                {
                    ...
                    if (generate)
                    {
                        if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) generating", Position, position);
                ...
                if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) acquiring streamLock", Position, position);
                streamLock.Wait();
                if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) streamLock acquired", Position, position);
                try
                {
                    if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) seeking to offset {Offset}", Position, position, chunkData.Item1);
                    ...
                    if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) compressionMode={Mode}", Position, position, compressionMode);
                    switch (compressionMode)
                    {
                        case 1: // gzip
                            throw new NotImplementedException("gzipped chunks are not implemented");
                        case 2: // zlib
                            if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) loading NBT zlib stream", Position, position);
                            var nbt = new NbtFile();
                            nbt.LoadFromStream(regionFile, NbtCompression.ZLib, null);
                            if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) NBT loaded, parsing Chunk", Position, position);
                            var chunk = Chunk.FromNbt(nbt);
                            chunk.ParentRegion = this;
                            Chunks[position] = chunk;
                            if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) chunk parsed", Position, position);
                ...
                {
                    streamLock.Release();
                    if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) streamLock released", Position, position);
                }
            }
        ...
        if (debug) _log.LogDebug("Region({Region}).GetChunk({Local}) done", Position, position);
        return Chunks[position];
    }
