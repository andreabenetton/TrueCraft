...
namespace TrueCraft.Launcher.Singleplayer
{
    public class SingleplayerServer
    {
        ...
        public SingleplayerServer(World world)
        {
            ...
            TrueCraft.Program.NodeConfiguration = new TrueCraft.NodeConfiguration
            {
                MOTD = null,
                Singleplayer = true
            };
        ...
        {
            Server.Log(LogLevel.Information, "Generating world around spawn point...");
            for (var x = -5; x < 5; x++)
            {
                for (var z = -5; z < 5; z++)
                ...
                if (progress % 10 == 0)
                    Server.Log(LogLevel.Information, "{0}% complete", progress + 10);
            }

            Server.Log(LogLevel.Information, "Simulating the world for a moment...");
            for (var x = -5; x < 5; x++)
            {
                for (var z = -5; z < 5; z++)
                {
